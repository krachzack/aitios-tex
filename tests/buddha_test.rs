...
use image::Rgba;
use scene::Entity;
use scene::{MaterialBuilder, Mesh};
use sim::{Simulation, SurfelData, TonSourceBuilder};
use std::fs::File;
use std::iter;
use std::rc::Rc;
...
#[test]
fn buddha_test() {
    let buddha = &obj::load("tests/assets/buddha.obj").expect("Could not load test geometry")[0];
    ...
    let mut sim = Simulation::new(vec![source], buddha.mesh.triangles(), surface, vec![]);
    ...
    let density = Density::new(
        ...
        Rgba {
            data: [0, 0, 255, 255],
        }, // color for texture space unused by the entity
        Rgba {
            data: [255, 255, 255, 255],
        }, // color for 0.0 density
        Rgba {
            data: [0, 0, 0, 255],
        }, // color for 1.0 density
    );
